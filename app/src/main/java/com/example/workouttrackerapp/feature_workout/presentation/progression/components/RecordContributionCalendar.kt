...
@Composable
fun RecordContributionCalendar(
    modifier: Modifier? = Modifier,
    exerciseString: String,
    contributionList: List<RecordContribution>,
    weekStartDay: WeekStartDay = WeekStartDay.SUNDAY,
    weightUnitLabel: WeightUnit = WeightUnit.KG,
    ...
    val headingTextColor =
        if (ColorUtils.calculateLuminance(MaterialTheme.colorScheme.surfaceVariant.toArgb()) < 0.5) {
            Color.White
        } else {
            Color.Black
        }
                    ...
                    Box(
                        ...
                        DropdownMenu(
                            ...
                            Column(
                                ...
                                modifier = Modifier
                                    .clickable {
                                        if (item.hasContribution && item.record != null) {
                                            isDropdownMenuVisible = false
                                            navController.navigate(
                                                AddEditWorkout(
                                                    item.record.workoutId,
                                                    item.record.workoutColor,
                                                    item.record.exerciseId
                                                )
                                            )
                                        }
                                    }
                                ...
                                val weightLabel: String =
                                    if (item.record?.avgWeight == null) {
                                        " : No data"
                                    } else {
                                        " : " + item.record.avgWeight.toString() + " " + weightUnitLabel.displayName.lowercase()

                                    }
                                val repsLabel: String =
                                    if (item.record?.avgReps == null) {
                                        " : No data"
                                    } else {
                                        " : " + item.record.avgReps.toString()
                                    }

                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    Icon(Icons.Default.FitnessCenter, "Weight")
                                    Text(weightLabel)
                                }
                                Row(verticalAlignment = Alignment.CenterVertically) {
